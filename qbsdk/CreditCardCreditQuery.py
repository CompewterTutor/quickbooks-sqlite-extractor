...
<QBXML>
    ...
    def from_response_xml(self, response_xml: str):
        ...
        cc_credit_rets = root.findall('.//CreditCardCreditRet')
        self.cc_credits = [CreditCardCreditRet.from_xml(etree.tostring(cc_credit_ret)) for cc_credit_ret in cc_credit_rets]
        self.cc_credits = [cc_credit for cc_credit in self.cc_credits if cc_credit is not None]

class CreditCardCreditRet(Base):
    __tablename__ = 'CreditCardCreditRet'
    ...
    def from_xml(xml_str: str):
        ...
        cc_credit_ret = root if root.tag == 'CreditCardCreditRet' else root.find('.//CreditCardCreditRet')
        if cc_credit_ret is None:
            return None
        ...
        return CreditCardCreditRet(
            txn_id=get_text(cc_credit_ret, 'TxnID'),
            time_created=datetime.fromisoformat(get_text(cc_credit_ret, 'TimeCreated')) if get_text(cc_credit_ret, 'TimeCreated') else None,
            time_modified=datetime.fromisoformat(get_text(cc_credit_ret, 'TimeModified')) if get_text(cc_credit_ret, 'TimeModified') else None,
            edit_sequence=get_text(cc_credit_ret, 'EditSequence'),
            txn_number=int(get_text(cc_credit_ret, 'TxnNumber')) if get_text(cc_credit_ret, 'TxnNumber') else None,
            account_list_id=get_text(cc_credit_ret, 'AccountRef/ListID'),
            account_full_name=get_text(cc_credit_ret, 'AccountRef/FullName'),
            payee_list_id=get_text(cc_credit_ret, 'PayeeEntityRef/ListID'),
            payee_full_name=get_text(cc_credit_ret, 'PayeeEntityRef/FullName'),
            ref_number=get_text(cc_credit_ret, 'RefNumber'),
            txn_date=datetime.fromisoformat(get_text(cc_credit_ret, 'TxnDate')) if get_text(cc_credit_ret, 'TxnDate') else None,
            amount=float(get_text(cc_credit_ret, 'Amount')) if get_text(cc_credit_ret, 'Amount') else None,
            currency_list_id=get_text(cc_credit_ret, 'CurrencyRef/ListID'),
            currency_full_name=get_text(cc_credit_ret, 'CurrencyRef/FullName'),
            exchange_rate=float(get_text(cc_credit_ret, 'ExchangeRate')) if get_text(cc_credit_ret, 'ExchangeRate') else None,
            amount_in_home_currency=float(get_text(cc_credit_ret, 'AmountInHomeCurrency')) if get_text(cc_credit_ret, 'AmountInHomeCurrency') else None,
            memo=get_text(cc_credit_ret, 'Memo'),
            is_tax_included=get_text(cc_credit_ret, 'IsTaxIncluded') == 'true' if get_text(cc_credit_ret, 'IsTaxIncluded') else None,
            sales_tax_code_list_id=get_text(cc_credit_ret, 'SalesTaxCodeRef/ListID'),
            sales_tax_code_full_name=get_text(cc_credit_ret, 'SalesTaxCodeRef/FullName'),
            external_guid=get_text(cc_credit_ret, 'ExternalGUID')
        )

class CreditCardCreditQueryRs:
    def __init__(self, status_code: int, status_severity: str, status_message: str, ret_count: int, cc_credit_ret: Optional[List[CreditCardCreditRet]] = None):
        self.status_code = status_code
        self.status_severity = status_severity
        self.status_message = status_message
        self.ret_count = ret_count
        self.cc_credit_ret = cc_credit_ret

def create_cc_credit_table_if_not_exists(engine):
    Base.metadata.create_all(engine)

def generate_cc_credit_query_xml(query: CreditCardCreditQueryRq) -> str:
    xml = f'''<?xml version="1.0" encoding="utf-8"?>
<?qbxml version="16.0"?>
<QBXML>
    <QBXMLMsgsRq onError="stopOnError">
